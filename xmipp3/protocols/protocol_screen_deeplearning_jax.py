...
class XmippProtScreenDeepLearningJax(ProtProcessParticles, XmippProtocol):
    """ Protocol for screening particles using deep learning. """
    _label = 'screen deep learning Jax'
    ...
    def _defineParams(self, form):
        ...
        form.addParam('batchSize', params.IntParam, default=32
                      help="A size of 32-64 should work for 256px images on 20GB GPUs",
                      label="Batch size"
                      )
    ...
    def train(self, posTrainDict, negTrainDict):
        ...
        args = " --pos %s --neg %s --batchSize %s --imgSize %d --gpu 0 --augment --bands 4" % (
            fnamesPos, fnamesNeg, self.batchSize.get(), self.resizeSize.get()
        )
        if not numberOfThreads is None:
            args += " --nCores %s" % (numberOfThreads)

        env = self.getCondaEnv()
        #env["LD_LIBRARY_PATH"] = "/home/miceta/software/miniforge/envs/xmipp_jax/lib:" + env["LD_LIBRARY_PATH"]
        self.runJob('xmipp_deep_consensus_jax_train', args, numberOfMpi=1,
                    env=env)
