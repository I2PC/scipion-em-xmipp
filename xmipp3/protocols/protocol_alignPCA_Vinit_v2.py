...
class XmippProtReconstructInitVolPca(ProtRefine3D, xmipp3.XmippProtocol):
    """Performs 3D initial volume generation based on PCA in fourier space and euclidean distance"""
    _label = 'alignPCA-Vinit'
    ...
    def _defineParams(self, form):
        ...
        form.addParam('correctCtf', BooleanParam, default=False, expertLevel=LEVEL_ADVANCED,
                      label='Correct CTF?',
    ...
    def globalAlign(self, inputXmd, output, angle, shift, MaxShift, applyShift, saveClass, rad):
        ...
        env=self.getCondaEnv()
        env['LD_LIBRARY_PATH'] = ''
        self.runJob("xmipp_alignPCA_Vinit", args, numberOfMpi=1, env=env)
