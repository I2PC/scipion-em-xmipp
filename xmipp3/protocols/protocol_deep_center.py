...
class XmippProtDeepCenterTrain(ProtAlign2D, xmipp3.XmippProtocol):
    """Train a neural network for deep centering"""
    _lastUpdateVersion = VERSION_3_0
    _conda_env = 'xmipp_DLTK_v1.0'
    _label = 'deep center training'
    ...
    def _defineParams(self, form):
        ...
        form.addParam('inputImageSet', PointerParam, label="Input images",
    ...
    def _insertAllSteps(self):
        ...
        numGPU = myStr.split(',')
        self._insertFunctionStep("convertInputStep", self.inputImageSet.get())
    ...
    def convertInputStep(self, inputSet):
        ...
        if self.trainSetSize.get()>0:
            self.runJob("xmipp_metadata_utilities","-i %s --operate random_subset %d"%\
                        (self.fnImgs,self.trainSetSize), numberOfMpi=1)

    def train(self, gpuId):
        args = "-i %s --omodel %s --sigma %f --maxEpochs %d --batchSize %d --gpu %s --learningRate %f --precision %f"%\
                (self.fnImgs, self._getExtraPath("model.h5"), self.sigma, self.numEpochs, self.batchSize, gpuId,
...
class XmippProtDeepCenterPredict(ProtAlign2D, xmipp3.XmippProtocol):
    """Apply a deep center model to align particles"""
    _lastUpdateVersion = VERSION_3_0
    _conda_env = 'xmipp_DLTK_v1.0'
    ...
    def _defineParams(self, form):
        ...
        form.addParam('modelProtocol', PointerParam, label='Protocol',
                      pointerClass='XmippProtDeepCenterTrain')
    ...
    def predict(self, gpuId):
        fnModel = self.modelProtocol.get()._getExtraPath("model.h5")
