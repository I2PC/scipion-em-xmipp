...
class XmippProtBoostParticles(EMProtocol):
    ...
    def boostingStep(self):
        ...
        if fnVol.endswith('.mrc'):
            fnVol += ':mrc'
        args = '-i %s --ref %s -o %s --sampling %f --max_resolution %f --padding %f --sigma %d --limit_freq %d --boost'\
    ...
    def _summary(self):
        summary = ["Volume: %s\nSet of particles: %s\n" %
                   (self.vol.get().getFileName(), self.particles.get())]
        return summary
