...
class XmippProtReconstructSwarm(ProtRefine3D):
    ...
    def _insertAllSteps(self):
        ...
        self._insertFunctionStep('calculateAverage',
                                 self.numberOfIterations.get() + 1)
    ...
    def convertInputVolume(self, imgHandler, obj, fnIn, fnOut, TsCurrent,
                           newXdim):
        self.runJob('xmipp_image_resize', "-i %s -o %s --factor %f" % (
        fnIn, fnOut, obj.getSamplingRate() / TsCurrent), numberOfMpi=1)
        objXdim, _, _, _ = imgHandler.getDimensions((1, fnOut))
        if newXdim != objXdim:
            self.runJob('xmipp_transform_window',
                        "-i %s --size %d" % (fnOut, newXdim), numberOfMpi=1)
    ...
    def convertInputStep(self, inputParticlesId):
        ...
        if self.nextMask.hasValue():
            self.convertInputVolume(imgHandler, self.nextMask.get(),
                                    getImageLocation(self.nextMask.get()),
                                    join(fnDir, "mask.vol"), TsCurrent, newXdim)

        # Prepare references
        i = 0
        for vol in self.inputVolumes.get():
            fnVol = join(fnDir, "volume%03d.vol" % i)
            self.convertInputVolume(imgHandler, vol, getImageLocation(vol),
                                    fnVol, TsCurrent, newXdim)
            self.runJob("xmipp_image_operate", "-i %s --mult 0 -o %s" % (
            fnVol, join(fnDir, "volume%03d_speed.vol" % i)), numberOfMpi=1)
            i += 1
        xmippLib.MetaData().write("best@" + self._getExtraPath(
            "swarm.xmd"))  # Empty write to guarantee this block is the first one
        xmippLib.MetaData().write(
            "bestByVolume@" + self._getExtraPath("swarm.xmd"),
            xmippLib.MD_APPEND)  # Empty write to guarantee this block is the second one
    ...
    def evaluateIndividuals(self, iteration):
        ...
        TsOrig = self.inputParticles.get().getSamplingRate()
        TsCurrent = self.readInfoField(fnDir, "sampling",
        ...
        bestIterVol = {}
        if iteration > 0:
            mdPrevious = xmippLib.MetaData(
                "bestByVolume@" + self._getExtraPath("swarm.xmd"))
            for objId in mdPrevious:
                idx = int(mdPrevious.getValue(xmippLib.MDL_IDX, objId))
                bestWeightVol[idx] = mdPrevious.getValue(xmippLib.MDL_WEIGHT,
                                                         objId)
        ...
        if iteration > 1:
            mdBest = xmippLib.MetaData(
                "best@" + self._getExtraPath("swarm.xmd"))
        ...
        for i in range(self.inputVolumes.get().getSize()):
            ...
            self.runJob("xmipp_transform_filter",
                        ...
                        fnVol, self.targetResolution, TsCurrent), numberOfMpi=1)
            if exists(fnMask):
                self.runJob("xmipp_image_operate",
                            "-i %s --mult %s" % (fnVol, fnMask), numberOfMpi=1)

            # Prepare subset of experimental images
            fnTest = join(fnDir, "imgs_%03d.xmd" % i)
            self.runJob("xmipp_metadata_utilities",
                        "-i %s --operate random_subset %d -o %s" % (
                        fnImages, self.NimgTest, fnTest), numberOfMpi=1)
		...
		count=0
            ...
            if exists(fnAngles):
                ...
                if avgWeight > bestWeight:
                    ...
                    mdBest.write("best@" + self._getExtraPath("swarm.xmd"),
                                 xmippLib.MD_APPEND)
        ...
        md.write(
            "evaluations_%03d@" % iteration + self._getExtraPath("swarm.xmd"),
            xmippLib.MD_APPEND)

        # Update best by volume
        if iteration == 0:
            md.write("bestByVolume@" + self._getExtraPath("swarm.xmd"),
                     xmippLib.MD_APPEND)
        else:
            md.clear()
            for i in range(self.inputVolumes.get().getSize()):
                objId = md.addObject()
                md.setValue(xmippLib.MDL_IDX, long(i), objId)
                md.setValue(xmippLib.MDL_IMAGE,
                            self._getExtraPath("volume%03d_best.vol" % i),
                            objId)
    ...
    def createOutput(self):
        ...
        self.runJob("xmipp_image_resize", "-i %s --dim %d" % (fnAvg, XdimOrig),
                    numberOfMpi=1)
        volume = Volume()
    ...
    def reconstructNewVolumes(self, iteration):
        ...
        for i in range(self.inputVolumes.get().getSize()):
            ...
            self.runJob("xmipp_metadata_utilities",
                        "-i %s --operate random_subset %d -o %s" % (
                        fnImages, self.NimgTrain, fnTrain), numberOfMpi=1)
    ...
    def postProcessing(self, iteration):
        ...
        for i in range(self.inputVolumes.get().getSize()):
            ...
            self.runJob('xmipp_volume_align',
                        '--i1 %s --i2 %s --local --apply' % (fnAvg, fnVol),
                        numberOfMpi=1)
            if iteration >= 2:
                fnVol = self._getExtraPath("volume%03d_best.vol" % i)
                self.runJob('xmipp_volume_align',
    ...
    def updateVolumes(self, iteration):
        ...
        for i in range(self.inputVolumes.get().getSize()):
            ...
            self.runJob("xmipp_image_operate", "-i %s --minus %s -o %s" % (
            fnVolBest, fnVol, fnInternal), numberOfMpi=1)
            self.runJob("xmipp_image_operate",
                        "-i %s --minus %s -o %s" % (fnBest, fnVol, fnExternal),
                        numberOfMpi=1)
            self.runJob("xmipp_image_operate",
                        "-i %s --mult %f" % (fnInternal, 2 * u1), numberOfMpi=1)
            self.runJob("xmipp_image_operate",
                        "-i %s --mult %f" % (fnExternal, 2 * u2), numberOfMpi=1)
            self.runJob("xmipp_image_operate",
                        "-i %s --plus %s" % (fnSpeed, fnInternal),
                        numberOfMpi=1)
            self.runJob("xmipp_image_operate",
                        "-i %s --plus %s" % (fnSpeed, fnExternal),
                        numberOfMpi=1)
            self.runJob("xmipp_image_operate",
                        "-i %s --plus %s" % (fnVol, fnSpeed), numberOfMpi=1)
    ...
    def calculateAverage(self, iteration):
        ...
        for i in range(self.inputVolumes.get().getSize()):
            if iteration <= 2:
                ...
                fnVol = self._getExtraPath("volume%03d_best.vol" % i)
                self.runJob("xmipp_image_operate",
                            "-i %s --plus %s" % (fnAvg, fnVol), numberOfMpi=1)
            N += 1
        if iteration > 0:
            self.runJob("xmipp_image_operate", "-i %s --plus %s" % (
            fnAvg, self._getExtraPath("volumeBest.vol")), numberOfMpi=1)
            N += 1
        self.runJob("xmipp_image_operate", "-i %s --divide %f" % (fnAvg, N),
                    numberOfMpi=1)
    ...
    def _methods(self):
        ...
        if hasattr(self, 'outputVolume') or True:
            ...
            strline += 'using %s as starting swarm and Xmipp swarm procedure. ' % (
