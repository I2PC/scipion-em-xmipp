    ...
    def evaluateReconstructions(self, iteration):
        ...
        if iteration > 0:
            ...
            self.runJob('xmipp_metadata_utilities',
                        ...
                        fnImagesId, fnUsedId, fnImagesRejected), numberOfMpi=1)
            self.runJob('xmipp_metadata_utilities',
    ...
    def prepareImages(self, fnDirPrevious, fnDir, TsCurrent, getShiftsFrom=''):
        ...
        if newXdim != Xdim:
            ...
            self.runJob("xmipp_image_convert",
                        ...
                        self.imgsFn, fnNewParticles, join(fnDir, "images.xmd")),
                        numberOfMpi=1)
        R = self.particleRadius.get()
        if R <= 0:
            R = self.inputParticles.get().getDimensions()[0] / 2
        ...
        if self.alignmentMethod == self.LOCAL_ALIGNMENT or \
            ...
            for i in range(1, 3):
                ...
                self.runJob("xmipp_metadata_utilities",
                            ...
                            fnLocalAssignment, fnLocalImagesIdx), numberOfMpi=1)
    ...
    def postProcessing(self, iteration):
        ...
        if self.postSoftNeg:
            ...
            self.runJob("xmipp_transform_filter",
                        ...
                        fnVol1, fnMask, fnFsc, TsCurrent, self.postSoftNegK),
                        numberOfMpi=1)
            self.runJob("xmipp_transform_filter",
                        "-i %s --softnegative %s %s %f %f" % (
                        fnVol2, fnMask, fnFsc, TsCurrent, self.postSoftNegK),
                        numberOfMpi=1)
            cleanPath(fnFsc)
            if removeMask:
                cleanPath(fnMask)
