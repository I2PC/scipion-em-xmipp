...
class UNET(DeepLearningModel):
  ...
  def __init__(self, boxSize, saveModelFname, gpuList="0", batchSize=BATCH_SIZE, modelDepth=4, generatorLoss="MSE",
                     trainingDataMode="ParticlesAndSyntheticNoise"):
  ...
  def train(self, learningRate, nEpochs, xmdParticles, xmdProjections):
    ...
    if os.path.isfile(self.saveModelFname):
      ...
      paramsNewUnet={"img_shape":self.img_shape, "out_ch":1, "start_ch":32, "depth": self.modelDepth,
                     ...
                     "maxpool":True, "upconv":True, "residual":True, "lastActivation":"linear"}      
      if N_GPUs>1:
        with tf.device('/cpu:0'):
...
def build_UNet( img_shape, out_ch=1, start_ch=32, depth=3, inc_rate=2., activation='relu', 
    dropout=0.5, batchnorm=False, maxpool=True, upconv=True, residual=False, lastActivation="linear"):
...
class WriteImageCBack(keras.callbacks.Callback):
  ...
  def on_epoch_end(self, epoch, logs={}):
      ...
      save_imgs([pred, self.x, self.y], ["generated", "particles", "projections"],
               self.saveImagesPath, self.currentEpoch)
