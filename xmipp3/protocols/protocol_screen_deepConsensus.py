...
#--------------------------- INSERT steps functions ---------------------------
    def _insertAllSteps(self):
        ...
        depsOr_cons= self.insertCaculateConsensusSteps( allMicIds, 'OR', prerequisites= depsPrepMic) #OR before noise always
        depsOr= self.insertExtractPartSteps( allMicIds, 'OR', prerequisites= depsOr_cons)
        depsTrain= depsOr
        if not self.doContinue.get()  or self.keepTraining.get():
            ...
            depsAnd= self.insertCaculateConsensusSteps( allMicIds, 'AND', prerequisites= depsOr)
    ...
    def insertCaculateConsensusSteps(self, micIds, mode, prerequisites): #TODO: make it parallel. It does not work due to concurrency and sqlite cursor
